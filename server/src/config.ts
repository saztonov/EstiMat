...
export const config = {
  ...
  ai: {
    ...
    // Потолок ОДНОВРЕМЕННЫХ запросов к шлюзу — на весь процесс, а не на задание. Считается общий
    // поток всех контуров (группировка, ИИ-чат, извлечение РД), а не поток каждого по отдельности.
    // Двойка — под лимиты прокси: у него одновременных запросов от нас 3 (1 в работе + 2 в
    // очереди, дальше 503 queue_full), у nginx перед ним — 5 соединений с адреса. Берём с запасом.
    maxConcurrency: Number(process.env.AI_MAX_CONCURRENCY || '2'),
    // Минимальный интервал между ОТПРАВКАМИ. Даёт не больше 40 запросов в любую минуту (у прокси
    // 60/мин) и меньше одного в секунду — счётчик nginx не начинает опустошаться, запас на всплеск
    // остаётся нетронутым. Рабочему потоку не мешает: там связывает одновременность (2 запроса по
    // 40 с — это 3 запроса в минуту). Нужен против шторма повторов, когда шлюз отвечает отказом за
    // доли секунды и слот освобождается мгновенно.
    minIntervalMs: Number(process.env.AI_MIN_INTERVAL_MS || '1500'),
    // Таймаут ОДНОЙ попытки. Обязан быть больше дедлайна прокси (190 с): оборвать запрос раньше
    // него — это 499 в его журнале и оплаченный ответ, который мы не прочитаем.
    attemptTimeoutMs: Number(process.env.AI_ATTEMPT_TIMEOUT_MS || '200000'),
    get enabled(): boolean {
  ...
  lmstudio: {
    ...
    // Только для загрузки каталога моделей (routes/llm). К вызовам самих моделей отношения не
    // имеет: там таймаут на попытку, и задаёт его вызывающий (см. lib/material-grouping/run).
