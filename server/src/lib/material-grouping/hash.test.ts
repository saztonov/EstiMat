...
test('эффективная версия промпта меняется от текста system, merge и режима noThink', () => {
  const v0 = computeEffectivePromptVersion('mg-2', 'system', 'merge', false);
  assert.equal(v0, computeEffectivePromptVersion('mg-2', 'system', 'merge', false), 'детерминизм');
  assert.notEqual(v0, computeEffectivePromptVersion('mg-2', 'SYSTEM', 'merge', false), 'правка system');
  assert.notEqual(v0, computeEffectivePromptVersion('mg-2', 'system', 'MERGE', false), 'правка merge');
  assert.notEqual(v0, computeEffectivePromptVersion('mg-2', 'system', 'merge', true), 'смена noThink');
  assert.ok(v0.startsWith('mg-2:'), 'читаемая базовая версия сохраняется в префиксе');
});

test('правка текста промпта инвалидирует input_hash через эффективную версию', () => {
  const v1 = computeEffectivePromptVersion('mg-2', 'system-A', 'merge', false);
  const v2 = computeEffectivePromptVersion('mg-2', 'system-B', 'merge', false);
