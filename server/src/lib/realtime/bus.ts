...
export class RealtimeRegistry {
   ...
   * серверная фоновая реакция, поэтому фильтрация по смете остаётся на стороне подписчика.
   * Потребителей сейчас нет: умная группировка от событий сметы отвязана (она ставится по
   * открытию раздела), а наружу метод не выставлен — decorate'а на fastify больше нет.
   */
