...
export async function exportSupplierOrderXlsx(
  ...
  const items = await pool.query<{ material_name: string; unit: string; quantity: string; cost_category_name: string | null }>(
    `SELECT material_name, unit, SUM(quantity)::numeric AS quantity, MIN(cost_category_name) AS cost_category_name
       FROM supplier_order_items
      WHERE order_id = $1
      GROUP BY material_name, unit
      ORDER BY cost_category_name NULLS LAST, material_name`,
    [orderId],
  );
  ...
  ws.columns = [{ width: 5 }, { width: 55 }, { width: 10 }, { width: 14 }, { width: 30 }];
  ...
  items.rows.forEach((it, i) => {
    ...
    const row = ws.addRow([i + 1, it.material_name, it.unit, qty, it.cost_category_name ?? '']);
    row.eachCell((cell) => {
      cell.border = { top: { style: 'thin' }, bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'thin' } };
    });
