...
// ---------- граница блока после раскрытой строки (isBoundaryAfter) ----------

test('граница после первой строки участка отрезает её в отдельный блок', () => {
  // Строка 0 раскрыта: её expanded-row не должен попасть внутрь объединения — блок обрывается на ней.
  assert.deepEqual(locationRowSpans(same(4), LOCATION_SPAN_MAX, (i) => i === 0), [1, 3, 0, 0]);
});

test('граница в середине участка режет его на два блока', () => {
  // Раскрыта строка 3 из участка 0..7 одного места → блоки [0..3] и [4..7].
  assert.deepEqual(
    blocks(locationRowSpans(same(8), LOCATION_SPAN_MAX, (i) => i === 3)),
    [4, 4],
  );
});

test('граница после последней строки блока размер не меняет', () => {
  // Последняя строка участка и так завершает блок — принудительная граница на ней ничего не сдвигает.
  assert.deepEqual(locationRowSpans(same(3), LOCATION_SPAN_MAX, (i) => i === 2), [3, 0, 0]);
});

test('несколько границ подряд дробят участок построчно', () => {
  assert.deepEqual(locationRowSpans(same(4), LOCATION_SPAN_MAX, () => true), [1, 1, 1, 1]);
});

test('граница не меняет полосы: место то же — цвет прежний', () => {
  // Полосы считаются по ключам, не по границам, поэтому раскрытие строки цвет блока не трогает.
  const keys = same(6);
  assert.deepEqual(
    locationStripes(keys),
    keys.map(() => false),
    'внутри одного места все строки в одной полосе, даже если блок разрезан раскрытием',
  );
});

test('сумма спанов покрывает все строки и при границах', () => {
  const keys = [...same(20), ...same(5, 'b')];
  const spans = locationRowSpans(keys, LOCATION_SPAN_MAX, (i) => i % 4 === 0);
  assert.equal(spans.reduce((s, v) => s + v, 0), keys.length);
  for (const size of blocks(spans)) assert.ok(size <= LOCATION_SPAN_MAX, `блок ${size} длиннее максимума`);
});
...
test('граница из withLocationBlocks обрывает объединение на раскрытой строке', () => {
  const four: Row[] = [
    { id: '1', place: 'a' },
    { id: '2', place: 'a' },
    { id: '3', place: 'a' },
  ...
  // Раскрыта строка 1 (id '2') → блоки [0..1] и [2..3].
  const out = withLocationBlocks(cols, four, keyOf, undefined, (i) => i === 1).columns;
  assert.equal(cellOf(out, 0, four).rowSpan, 2);
  assert.equal(cellOf(out, 1, four).rowSpan, 0);
  assert.equal(cellOf(out, 2, four).rowSpan, 2);
  assert.equal(cellOf(out, 3, four).rowSpan, 0);
});
