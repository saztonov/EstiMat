...
export function RequestLotsSection({ requestId }: { requestId: string }) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get<{ data: { lots: LotBrief[]; coverage: Coverage } }>(`/supplier-orders/by-request/${requestId}`),
  });
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }} size="small">
      ...
      {lots.length === 0 ? (
        ...
        <Table<LotBrief> rowKey="id" size="small" loading={isLoading} pagination={false} dataSource={lots} columns={columns} scroll={{ x: 900 }} />
      )}
    </Space>
  );
}
