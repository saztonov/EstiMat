...
/**
 * «примерно через 20 мин» — грубый срок отложенного пересчёта. Именно грубый: плашка живёт без
 * таймера и перерисовывается только при перечитывании роута, а секунды на экране, тикающие раз в
 * полчаса, были бы просто неправдой.
 */
export function formatRoughCountdown(at: string | null, now: number): string | null {
  if (!at) return null;
  const left = new Date(at).getTime() - now;
  if (!Number.isFinite(left) || left <= 0) return null;
  const min = Math.ceil(left / 60_000);
  return min < 2 ? 'меньше чем через минуту' : `примерно через ${min} мин`;
}
...
/** Когда пересчёт возможен снова и кто его закажет. Обещать больше, чем сервер сделает, нельзя. */
export interface AutoRunContext {
  /** Пересчёт отложен до этого момента (прошлый прогон слишком свежий). */
  nextAutoRunAt: string | null;
  now: number;
}

/**
 * Почему пересчёта не будет — или когда он будет. Раньше панель безусловно обещала «Пересчёт
 * запустится автоматически»: после остановки это была неправда, а теперь неправдой стало и само
 * слово «автоматически» — расчёт заказывает открытый раздел, а не правка сметы.
 */
export function suppressedNotice(
  ...
  // Расчёт был совсем недавно: пересчитывать смету после каждой правки — это часы работы модели
  // впустую, поэтому следующий прогон отложен. Раздел останется открытым — начнётся сам.
  const when = formatRoughCountdown(auto.nextAutoRunAt, auto.now);
  if (when) {
    return {
      type: 'warning',
  ...
  return {
    ...
    description:
      'Показан прежний результат — он может не соответствовать текущему списку материалов. ' +
      'Пересчёт запустится при следующем открытии раздела.',
  };
}
