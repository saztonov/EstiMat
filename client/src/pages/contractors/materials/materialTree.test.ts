...
test('узел уровня «Вид работ» несёт costTypeId, остальные — null', () => {
  const rows = rowsOf();
  const levels: MaterialLevelSettings = { costType: true, location: true, locationType: true };
  const tree = buildMaterialTree(rows, levels);

  // Корни — категории: у них вида работ нет.
  for (const cat of tree) assert.equal(cat.costTypeId, null, 'узел категории');

  const costTypeNodes = tree.flatMap((cat) => cat.children);
  assert.ok(costTypeNodes.length > 0, 'уровень вида работ построен');
  for (const n of costTypeNodes) {
    assert.equal(n.level, 'costType');
    // Ключ узла — путь вида 'cat:<id>/ct:<id>': id в поле обязан совпадать с ним.
    assert.equal(n.costTypeId, n.key.split('/ct:')[1] || null);
    // Ниже вида работ (локация/тип) поле снова пустое.
    for (const child of n.children) assert.equal(child.costTypeId, null, 'узел под видом работ');
  }
  assert.deepEqual(
    costTypeNodes.map((n) => n.costTypeId).sort(),
    ['ct-heat', 'ct-water'],
  );
});
